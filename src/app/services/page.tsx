...
export default function ServicesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F6F4FF] text-[#040404]" dir="rtl">
        ...
        <div className="relative flex items-center justify-between" dir="ltr">
          ...
          {menuOpen && (
            <div className="absolute top-[calc(100%+10px)] left-0 right-0 bg-[#FFFFFFF2] backdrop-blur-md border border-[#5F30EB22] rounded-3xl flex flex-col items-center py-6 space-y-4 md:hidden z-50">
              ...
              <Link href="/GetStarted?mode=login" className="text-[#040404] hover:text-[#5F30EB] transition-colors">
                Log In
              </Link>
              <Link href="/GetStarted?mode=signup"
      ...
      {/* Services Grid */}
      <section className="max-w-5xl mx-auto px-4 pb-24 grid grid-cols-1 md:grid-cols-2 gap-8">
        {SERVICES.map((service) => (
          <Link
            key={service.slug}
            href={`/services/${service.slug}`}
            className="group bg-white rounded-3xl border border-[#5F30EB]/10 overflow-hidden hover:border-[#5F30EB]/40 hover:shadow-[0_8px_40px_rgba(95,48,235,0.12)] transition-all duration-300"
          >
            {/* Image */}
            <div className="relative h-52 bg-[#F0EEFF] overflow-hidden">
              {service.image ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={service.image}
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24"
                    fill="none" stroke="#5F30EB" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" className="opacity-30">
              ...
              {/* Price badge */}
              <div className="absolute top-4 left-4 bg-[#5F30EB] text-white text-sm font-bold px-4 py-1.5 rounded-full">
                {service.price}
              </div>
            </div>
            ...
            {/* Content */}
            <div className="p-6">
              <h2 className="text-xl font-bold text-[#040404] mb-2 leading-snug">
                {service.title}
              </h2>
              <p className="text-sm text-[#6A6A82] mb-5 leading-relaxed">
                {service.subtitle}
              </p>
              <ul className="space-y-2 mb-6">
                {service.features.map((f) => (
                  <li key={f} className="flex items-center gap-2 text-sm text-[#4F4F63]">
      ...
      <footer className="relative overflow-hidden text-[#040404] pt-20 pb-10 border-t border-[#5F30EB]/20" dir="ltr">
        ...
        <div className="max-w-7xl mx-auto flex px-4 justify-between flex-col gap-4 lg:flex-row lg:items-center">
          ...
          <div className="flex flex-row items-center justify-start gap-8 text-sm text-[#6A6A82]">
            ...
            <Link href="/services" className="hover:text-[#5F30EB] transition-colors">Services</Link>
